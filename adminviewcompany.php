...
<head>
    ...
    <title>Admin Dashboard - View Companies</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        table { width: 100%; border-collapse: collapse; text-align: left; font-size: 14.5px; min-width: 900px; }
        ...
        .type-badge { background: #f1f5f9; color: #475569; font-size: 12.5px; font-weight: 600; padding: 4px 10px; border-radius: 6px; display: inline-block; }
        ...
        .delete-btn { background-color: #fdf2f2; color: #de3e3e; border: 1px solid rgba(222, 62, 62, 0.1.5); }
    ...
    <div class="dashboard-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-title-section">
                <h2>Corporate Directorship</h2>
                <p>Review, update credentials, or securely manage corporate profiles registered within the network.</p>
            </div>

            <!-- Table Block Card -->
            <div class="table-card">
                <h3 style="font-size: 17px; font-weight: 700; color: #1e293b; margin-bottom: 5px;">Registered Accounts</h3>
                            ...
                            if ($result && mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr>
                                        ...
                                        <td style='font-weight: 600; color: #111;'> " . htmlspecialchars($row['company_name']) . "</td>
                                        <td>" . htmlspecialchars($row['username']) . "</td>
                                        <td>" . htmlspecialchars($row['email']) . "</td>
                                        <td>" . htmlspecialchars($row['address']) . "</td>
                                        <td><code style='font-family: monospace; font-size: 13px; color: #64748b;'>" . htmlspecialchars($row['company_pan']) . "</code></td>
                                        <td>" . htmlspecialchars($row['company_license']) . "</td>
                                        <td><span class='type-badge'>" . htmlspecialchars($row['company_type']) . "</span></td>
                                        <td align='center'>
                                            <div class='action-holder'>
                                                <button class='action-btn edit-btn' onclick=\"window.location.href='adminupdatecompany.php?cid={$row['cid']}'\">Edit</button>
                                                <button class='action-btn delete-btn' onclick=\"if(confirm('Are you sure you want to permanently delete this corporate entry?')) window.location.href='admindeletecompany.php?cid={$row['cid']}';\">Delete</button>
                                            </div>
                                        </td>
                                    </tr>";
                                }
                            } else {
                                echo "<tr><td colspan='9' style='text-align:center; color:#888; padding: 30px;'>No corporate organizations found inside the database grid.</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
